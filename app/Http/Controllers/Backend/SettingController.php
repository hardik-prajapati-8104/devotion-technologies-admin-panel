...
class SettingController extends Controller
{
    ...
    private const GENERAL_KEYS = ['site_name', 'site_email', 'site_phone', 'site_address', 'copyright_text'];
    ...
    private const SMTP_KEYS    = ['smtp_host', 'smtp_port', 'smtp_username', 'smtp_encryption', 'smtp_from_email', 'smtp_from_name'];
    ...
    public function updateGeneral(Request $request)
    {
        ...
        if ($request->hasFile('site_logo')) {
            Setting::set('site_logo', $this->imageUploadService->replace($request->file('site_logo'), 'settings', Setting::get('site_logo')), 'general');
        }

        if ($request->hasFile('site_favicon')) {
            Setting::set('site_favicon', $this->imageUploadService->replace($request->file('site_favicon'), 'settings', Setting::get('site_favicon')), 'general');
        }
     ...
     * form (see the Blade view — the password field is always left
     * blank, like an admin's own password field). Leaving it blank on
     * submit keeps the existing encrypted value.
     */
